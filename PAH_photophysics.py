...
def plot_acetylene_dissociation_rate(G0min,G0max,nHmin,nHmax):
...
def plot_dissociation_rates():
    ...
    dissH_dehidrogenated = pd.read_csv('andrews16_Hdissociation_rates_C54H17.csv',names=['E','k'])
    axes[0].plot(dissH_dehidrogenated['E'],dissH_dehidrogenated['k'],linestyle='-',color='k')
    dissH_hidrogenated = pd.read_csv('andrews16_Hdissociation_rates_C54H18.csv',names=['E','k'])
    axes[1].plot(dissH_hidrogenated['E'],dissH_hidrogenated['k'],linestyle='-',color='k')
    dissH2_hidrogenated = pd.read_csv('andrews16_H2dissociation_rates_C54H18.csv',names=['E','k'])
    axes[1].plot(dissH2_hidrogenated['E'],dissH2_hidrogenated['k'],linestyle='--',color='k')
    dissH_superhidrogenated = pd.read_csv('andrews16_Hdissociation_rates_C54H19.csv',names=['E','k'])
    axes[2].plot(dissH_superhidrogenated['E'],dissH_superhidrogenated['k'],linestyle='-',color='k')
    
    # 2. Load the Castellanos et al. (2018) data
    dissoddH_cation = pd.read_csv('castellanos18_oddHdissociation_rates_C54H18.csv',names=['E','k'])
    axes[1].plot(dissoddH_cation['E'],dissoddH_cation['k'],linestyle=':',color='b')
    dissH_cation = pd.read_csv('castellanos18_Hdissociation_rates_C54H18.csv',names=['E','k'])
...
def compute_h2_dissociation_rate(args):
    from PAH_photoelectric_heating import ionisation_potential,ionisation_yield
...
def plot_h2_dissociation_rate(G0min,G0max,n_G0=100):
    ...
    # 2. Compute the rate for the circumcoronene neutral
    dist = LogNormal_Distribution(basic_a0[0],basic_amin[0],basic_amax[0],basic_sigma[0],basic_s[0])
    dist.Nc = 54
    Z = 0
...
def plot_h2_efficiency(sizePAH,G0,ntmin,ntmax,Xe,T,f,n_nt=100):
    from PAH_photoelectric_heating import read_data,interpolate_linear
    from scipy.optimize import curve_fit
    import matplotlib.pyplot as plt
    import seaborn as sns
    sns.set_theme(style="white")
    ...
    fig.savefig(f'H2_pah_efficiency_{sizePAH}.pdf',format='pdf',dpi=300)

def plot_h2_formation(model,G0,ntmin,ntmax,Xe,T,f,xPAH=3.3e-5,n_nt=100):
    from PAH_photoelectric_heating import read_data,interpolate_linear,recombination_rate_Spitzer,recombination_rate_Tielens21
    from scipy.optimize import curve_fit
    import matplotlib.pyplot as plt
    import seaborn as sns
    sns.set_theme(style="white")
    ...
    wav,sigma_abs_dication = absorption_cross_section(dist_small,1)
    E0 = np.array([dissociation_parameters['Murga2020']['dehydrogenated']['H(Z<= 0)']['E0'],
          dissociation_parameters['Murga2020']['dehydrogenated']['H2']['E0']])
    S = np.array([dissociation_parameters['Murga2020']['dehydrogenated']['H(Z<= 0)']['S'],
          dissociation_parameters['Murga2020']['dehydrogenated']['H2']['S']])
    k_H2 = np.array([compute_h2_dissociation_rate((wav,sigma_abs_anion,G0,E0,S,dist_small,-1)),
                     compute_h2_dissociation_rate((wav,sigma_abs_neu,G0,E0,S,dist_small,0)),
                     compute_h2_dissociation_rate((wav,sigma_abs_cation,G0,E0,S,dist_small,1)),
                     compute_h2_dissociation_rate((wav,sigma_abs_dication,G0,E0,S,dist_small,2))])
    
    # 5. Compute the collisional rates
    k_col = np.array([7.8e-10 * nH,
                      nH * np.pi*(dist_small.a0*1e-4)**2.*np.sqrt(8.*kb*T/(np.pi*mh))*np.exp(-60*1e-3*eV2erg/(kb*T)),
                      1.4e-10 * nH,
                      1.4e-10 * nH])
