...
from dust_charging import compute_charge_vs_gamma

if __name__ == '__main__':
    grain_type = 'graphite'
    a_micron = 0.005
    gamma_values = np.logspace(-4, 6, 150)
    results, fig = compute_charge_vs_gamma(
        grain_type, a_micron, gamma_values,
        combos_per_gamma=100, seed=1, debug=False,
        temp_bin_edges=np.logspace(1, 7, 31)
    )
    print('Done.')
    ...
    fig2.tight_layout()
    outpath = f'examples/gamma_scan_sixcols_{grain_type}_a{a_micron:.4g}um.png'
