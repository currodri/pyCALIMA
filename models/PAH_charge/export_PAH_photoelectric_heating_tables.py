                ...
                for att_model in attachment_models:
                    try:
                        ...
                        compute_tables_ISRF(
                            ...
                            file_prefix=f'{pah_bin_id}_',
                        )
                        
                        # Register created files written directly into output_root.
                        created_files.extend(str(path) for path in output_root.glob(f'{pah_bin_id}_peh_Pinj_ISRF_{rad_model}_{opt_model}_{att_model}_*.dat'))
                        created_files.extend(str(path) for path in output_root.glob(f'{pah_bin_id}_peh_Pinj_ISRF_{rad_model}_{opt_model}_{att_model}_*.pdf'))
