...
Default method (Method B): compute_branching_integrated_rates with no table args.
  Uses GD89 f(T) and RRKM k(E) computed entirely from PAHdb modes.

Montillaud (2013) method: compute_branching_integrated_rates_montillaud
  Three-channel competition: H-loss + H2-loss + C2H2-loss.
  Uses Montillaud+13 / Berne & Tielens (2012) RRKM parameters from pah_h_state.
  IR cooling follows the GD89 QHO model (unchanged).

Reference methods (require Andrews k(E) tables):
  compute_andrews_direct_branching  (Method E)
  compute_bakes_direct_branching    (Method D single-photon)
  compute_bakes_dwek_branching      (Method D Volterra / Dwek multi-photon)

Utility:
  compare_dissociation_methods  — runs all methods for a G0 grid and returns results dict
  print_method_comparison       — tabular printout vs Andrews digitised data
"""
...
def compute_total_dissociation_rate(file_path, t_centers, f_T, E_act_ev, dS_cl_jk,
    ...
    """
    Total time-averaged dissociation rate ∫ f(T) × K_dis(T) dT [s^-1].

    Used as the component rates for Method A (then scaled by R_abs and branching).
    """
...
def compute_branching_integrated_rates(file_path, t_centers, f_T,
    ...
    """
    Time-averaged H-loss and H2-loss branching rates [s^-1].

    At each temperature bin the competition between H-loss, H2-loss, and IR
    cascade is resolved microcanonically, then integrated over f(T).

    Default (no table args): Method B — RRKM rates from PAHdb modes.
    With table args: uses Andrews k(E) tables (Method C when all three supplied).

    Returns (Y_H, Y_H2) in s^-1.
    """
...
# ---------------------------------------------------------------------------
# Montillaud (2013) three-channel branching: H + H2 + C2H2
# ---------------------------------------------------------------------------

def compute_branching_integrated_rates_montillaud(
    file_path, t_centers, f_T,
    E_act_H,    dS_H,
    E_act_H2,   dS_H2,
    E_act_C2H2, dS_C2H2,
    t_min=15.0,
    k_H_table=None, k_H2_table=None, k_C2H2_table=None, k_IR_table=None,
    u_centers=None,
):
    """
    Montillaud, Joblin & Toublanc (2013) three-channel branching rates [s^-1].

    Extends compute_branching_integrated_rates by adding a C2H2-loss (acetylene)
    channel following Berne & Tielens (2012) / Allain (1996).  All three
    dissociation channels compete with each other and with IR cooling at every
    temperature bin.

    The IR cooling rate per photon absorbed is:

        k_IR(T) = [Σ_i A_i × ħν_i × n_i(T)] / U(T)

    where n_i(T) = 1/(exp(ħν_i/k_B T) − 1) is the Planck–QHO occupation.
    This is the Guhathakurta & Draine (1989) / Desert et al. (1986) prescription
    used unchanged from Montillaud (2013) Sect. 2.2.1.

    The RRKM dissociation rate for channel j at vibrational temperature T is:

        k_j(T) = exp(1) × (k_B T_e / h) × exp(ΔS_j / R) × exp(−E_{0,j} / k_B T_e)

    where T_e = T × (1 − 0.2 × E_{0,j} / U(T)) is the Tielens (2005) effective
    temperature and U(T) is the internal energy from QHO modes.

    The cascade-corrected branching for each bin (GD89 W_down convention):

        B_j(T) = k_j × W_down / (k_H + k_H2 + k_C2H2 + W_down)

    where W_down = k_IR × U / ΔU is the effective IR de-excitation rate across
    the bin.

    Parameters
    ----------
    file_path : str
        Path to the PAHdb transitions file (provides modes and Einstein-A).
    t_centers, f_T : ndarray
        Temperature bin centres [K] and the f(T) probability density [K^-1]
        from compute_adaptive_temperature_distribution.
    E_act_H, dS_H : float
        H-loss activation energy [eV] and activation entropy [J/mol/K].
    E_act_H2, dS_H2 : float
        H2-loss activation energy [eV] and activation entropy [J/mol/K].
    E_act_C2H2, dS_C2H2 : float
        C2H2-loss (acetylene) activation energy [eV] and entropy [J/mol/K].
    t_min : float
        Lower temperature cutoff [K]; bins below are skipped.
    k_H_table, k_H2_table, k_C2H2_table : str or None
        Paths to two-column CSV [E_eV, k_s-1] tables.  When not None, the
        tabulated RRKM rates replace the analytical RRKM formula for that
        channel (useful for validation against Andrews 2016 tables).
    k_IR_table : str or None
        Path to a two-column CSV for the IR cooling rate.  When None,
        the QHO formula is used (default Montillaud 2013 treatment).
    u_centers : ndarray or None
        Pre-computed U(T) at t_centers.  Avoids redundant QHO summations when
        calling this function repeatedly with the same molecule.

    Returns
    -------
    (Y_H, Y_H2, Y_C2H2) : (float, float, float)
        Time-averaged H-loss, H2-loss, and C2H2-loss branching rates [s^-1].
    """
    freq_ev, einstein_A = load_pah_modes(file_path)
    ...
    # --- Internal energy at edges (for W_down) ---
    def _u_arr(T_arr):
        result = np.zeros(len(T_arr))
        for idx, T in enumerate(T_arr):
            if T <= 0:
    ...
    for j in range(num_bins):
        ...
        # ---- IR cooling rate (Montillaud 2013 Eq. 3 / GD89) ----
        if k_IR_table is not None:
            k_IR = compute_dissociation_rate_from_table(k_IR_table, U_Tj)
        else:
            x = freq_ev / (_KB_EV * T_j)
            with np.errstate(over='ignore', under='ignore', invalid='ignore'):
                occ = np.where(x > 50.0, np.exp(-x) / (1.0 - np.exp(-x)), 1.0 / np.expm1(x))
                occ = np.where(np.isnan(occ) | np.isinf(occ), 0.0, occ)
            # k_IR = P_IR / U  [s^-1] (energy-normalised IR emission rate)
            k_IR = float(np.sum(freq_ev * einstein_A * occ) / U_Tj)

        # ---- H-loss rate (Montillaud 2013 Eq. 4 / Tielens 2005) ----
        if k_H_table is not None:
            k_H = compute_dissociation_rate_from_table(k_H_table, U_Tj)
        else:
            if U_Tj >= E_act_H:
        ...
        # ---- C2H2-loss rate (Montillaud 2013 / Berne & Tielens 2012) ----
        if k_C2H2_table is not None:
            k_C2H2 = compute_dissociation_rate_from_table(k_C2H2_table, U_Tj)
        else:
            if U_Tj >= E_act_C2H2:
                T_e3 = T_j * (1.0 - 0.2 * E_act_C2H2 / U_Tj)
                k_C2H2 = (np.exp(1.0) * (_KB_J_K * T_e3 / _H_J_S)
                          * np.exp(dS_C2H2 / _R_GAS)
                          * np.exp(-E_act_C2H2 / (_KB_EV * T_e3))) if T_e3 > 0 else 0.0
            else:
                k_C2H2 = 0.0
        ...
        du_j     = delta_u[j] if j < len(delta_u) else delta_u[-1]
        W_down_j = (k_IR * U_Tj / du_j) if du_j > 0 else k_IR
        denom    = k_H + k_H2 + k_C2H2 + W_down_j
        if denom > 0:
            Y_H_arr[j]    = k_H    * W_down_j / denom
            Y_H2_arr[j]   = k_H2   * W_down_j / denom
            Y_C2H2_arr[j] = k_C2H2 * W_down_j / denom

    return (
        float(np.sum(f_disc * Y_H_arr)),
        float(np.sum(f_disc * Y_H2_arr)),
        float(np.sum(f_disc * Y_C2H2_arr)),
    )
...
def compute_andrews_direct_branching(radiation_field_func, cross_section_table,
    ...
    """
    Method E: direct energy-weighted branching (Andrews 2016).
...
def compute_bakes_direct_branching(radiation_field_func, cross_section_table,
                                   ...
                                   E_min=6.0, E_max=13.6, n_eval=800):
    """
    Method D (single-photon): Bakes (2001) / Andrews (2016) thermal model.

    Y_H = ∫ k_H(U)/P_IR(U) × R_abs(>U) dU
    """
    hc_ev  = _HC_EV
    h_cgs  = 6.62607015e-27
    c_cgs  = 2.99792458e10
...
def compute_bakes_dwek_branching(radiation_field_func, cross_section_table,
                                 ...
                                 E_min=6.0, E_max=13.6, n_eval=800):
    """
    Method D + Dwek (1986) multi-photon corrections (Volterra integral equation).

    Solves I_total(U) = R_above(U) + ∫ [I_total(U')/P_IR(U')] × R_above(U-U') dU'

    Reduces to compute_bakes_direct_branching when SI << 1.
    """
    hc_ev  = _HC_EV
    h_cgs  = 6.62607015e-27
    c_cgs  = 2.99792458e10
