...
Dissociation methods available
-------------------------------
Method B (default): compute_branching_integrated_rates
  Two-channel (H + H2) competition with IR cooling.  Uses Andrews (2016) RRKM
  parameters and the GD89 f(T) probability distribution from PAHdb modes.

Method M (Montillaud 2013): compute_branching_integrated_rates_montillaud
  Three-channel (H + H2 + C2H2) competition with IR cooling.  Adds the
  acetylene-loss channel following Montillaud, Joblin & Toublanc (2013)
  A&A 552 A15 and Berne & Tielens (2012).  The C2H2 channel becomes dominant
  for fully dehydrogenated PAHs at high G0 and is the primary photodestruction
  pathway in intense radiation fields.

  RRKM parameters (Montillaud 2013 / Berne & Tielens 2012):
    H-loss   (dehyd.)  : E_0 = 4.30 eV, ΔS = 49.4  J/(mol K)
    H2-loss             : E_0 = 3.52 eV, ΔS = −53.1 J/(mol K)
    C2H2-loss (dehyd.) : E_0 = 4.60 eV, ΔS = 41.8  J/(mol K)

  IR cooling: unchanged — GD89 QHO model (Guhathakurta & Draine 1989).
    k_IR(E) = P_IR(E)/E = [Σ_i A_i ħν_i n_i(T_vib)] / U(T_vib)
  where T_vib is found by inverting E = Σ_i ħν_i n_i(T_vib) using PAHdb modes.
"""
