...
def build_pahdb_catalog(xml_path: str, catalog_path: str,
                        nc_values: list = None) -> list:
    """
    Parse the PAHdb XML and write a JSON catalog of all non-radical singlet
    CnHm species (optionally filtered to nc_values).

    Only closed-shell singlets (spin multiplicity = 1) are included.
    Bare carbon (Nh=0) species are included if they are singlets.
    ...
    with open(xml_path, 'r', errors='replace') as f:
        for line in f:
            ...
            if '</specie>' in line:
                text = ''.join(buffer)
                entry = _parse_specie_block(text, cur_uid, nc_set)
...
def _parse_specie_block(text: str, uid: str, nc_set) -> dict | None:
...
def find_best_species(catalog: list, Nc: int, Nh: int, Z: int) -> tuple:
    ...
    if not pool:
        return None, False, f"No singlet species with Nc={Nc} Z={Z:+d} in catalog"
