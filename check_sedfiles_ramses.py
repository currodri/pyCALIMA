...
def main():
    ...
    SED_ages, SED_zeds, _ = read_sed_file(first_file,ndust)
    SED_nA = len(SED_ages)
    SED_nZ = len(SED_zeds)
    num_photon_groups = len(file_list)
    
    # Initialize combined SED array
    combined_sed = np.zeros((SED_nA, SED_nZ, ndust, num_photon_groups))
    photon_groups = []
    ...
    for file in file_list:
        ...
        print(file)
        _, _, SED_table_dust = read_sed_file(file,ndust)
        combined_sed[:, :, :, ip-1] = SED_table_dust[:,:,:]
    
    # Plot the combined SED
    print('I am here')
    plot_combined_sed(name,SED_ages, SED_zeds, ndust, combined_sed, photon_groups)
