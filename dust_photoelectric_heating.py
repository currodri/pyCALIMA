...
from dust_model import grain_charge_dist,grain_mean_charge,grain_charge_probability
...
def ionisation_potential_valence(W,Z,a):
    ...
    ----------
    W : float
        Work function of the material (in eV).
    Z : int or float
        Charge of the grain (in units of elementary charge).
    a : float
        Radius of the grain (in nm).
    Returns
    -------
    float
        Ionisation potential (in eV), calculated according to Eq. 2 of Weingartner & Draine (2001).
    References
    ----------
    Weingartner, J. C., & Draine, B. T. (2001). Photoelectric emission from interstellar dust: Grain charging and gas heating. The Astrophysical Journal Supplement Series, 134(2), 263.

    """    
    # Eq 2 Weingartner & Draine 2001
    return W + e/(4.*np.pi*epsilon_0) * ((Z + 0.5) / a + (Z+2.)/a * (0.03/a))
...
def plot_ionisation_potential(W):
    ...
    plt.savefig('ionisation_potential.pdf',format='pdf',dpi=300)
...
def plot_ionpot_vs_charge():
    ...
    plt.savefig('ionisation_potential_vs_charge.pdf',format='pdf',dpi=300)



def electron_affinity_graphite(W, Z, a):
    # Eq 4 Weingartner & Draine 2001
    return W + e/(4.*np.pi*epsilon_0) / a * ((Z-0.5) - 0.4 / (a+0.7))

def electron_affinity_silicate(W, Z, a):
    # Eq 5 Weingartner & Draine 2001
    return W - silicate_band_gap + e/(4.*np.pi*epsilon_0) * (Z-0.5) / a
...
def plot_electron_affinity():
    ...
    a = (Nc/468.)**(1./3.) # [nm]
    Z = 0
    EA = graphite_work_function + e/(4.*np.pi*epsilon_0) / a * ((Z-0.5) - 0.4 / (a+0.7))
    plt.figure(figsize=(8,6))
    plt.ylim(-1,3)
    plt.xlabel(r'$N_C$',fontsize=16)
    ...
    plt.legend(fontsize=12,frameon=False)
    plt.savefig('electron_affinity_graphite.pdf',format='pdf',dpi=300)

def min_energy_ejection(Z,a):
    # Eq 7 Weingartner & Draine 2001
    if Z >= 0:
        return 0.0
    else:
        return - e/(4.*np.pi*epsilon_0) * (Z+1.) / a / (1.+(2.7/a)**.75)

def photodetachment_energy_graphite(Z,a):
    # Eq 18 Weingartner & Draine 2001
    Emin = min_energy_ejection(Z,a)
    E = electron_affinity_graphite(graphite_work_function,Z+1,a)
    return E + Emin

def photodetachment_energy_silicate(Z,a):
    # Eq 18 Weingartner & Draine 2001
    Emin = min_energy_ejection(Z,a)
    E = electron_affinity_silicate(silicate_work_function,Z+1,a)
    return E + Emin
...
def parameter_theta(E,Emin_ej,Z,a):
    # Eq 9 Weingartner & Drain 2001
    if Z >= 0:
        theta = E - Emin_ej + e/(4.*np.pi*epsilon_0) * (Z+1.) / a
    else:
        theta = E - Emin_ej
...
def attempting_electron_energy_dist(E,hnu,Emin,Emin_ej,Z,a):
    ...
    if Z < 0:
        ...
        Elow = - e/(4.*np.pi*epsilon_0) * (Z+1.) / a
        Ehigh = hnu - Emin_ej
...
def photon_attenuation_length_graphite(wav,Imperp,Impar):
    # Eq 15 Weingartner & Draine 2001
    l = 4. * np.pi / wav * (2./3. * Imperp + 1./3. * Impar)
    return 1./l

def photon_attenuation_length_silicate(wav,Im):
    # Eq 14 Weingartner & Draine 2001
    l = wav / (4.*np.pi*Im)
    return l

def Watson73_y1(a,la,le):
    # Eq 13 Weingartner & Draine 2001

    beta = a / la
    alpha = a / le + a / la
    y1 = (beta/alpha)**2. * (alpha**2.-2.*alpha+2.-2.*np.exp(-alpha)) \
             / (beta**2.-2.*beta+2.-2.*np.exp(-beta))
    # if RuntimeWarning occurs, print the values
    if np.any(np.isnan(y1)) or np.any(np.isinf(y1)):
        print(f"Warning: y1 computation resulted in NaN or Inf for a={a}, la={la}, le={le}.")
        print(f"    alpha={alpha}, beta={beta}, y1={y1}")

    return y1

def BT94_y0_graphite(theta,W):
    # Eq 16 Weingartner & Draine 2001
    y0 = 9e-3 * (theta/W)**5. / (1. + 3.7e-2*(theta/W)**5.)

    return y0

def y0_silicate(theta,W):
    # Eq 17 Weingartner & Draine 2001
    # Using a fitting to the scaling measured
    # by Feuerbacher et al. (1972)

    y0 = 0.5 * (theta/W) / (1. + 5. * (theta/W))
...
def plot_dielectric_data(filename):
    ...
    plt.yscale('log')
    plt.grid()
    plt.savefig(f'{filename}.png',format='png',dpi=200)
...
def plot_photoelectric_yields(grain_types,a,Z,nE=100):
    ...
    wavelengths = 1.2398 / photon_energies # [micron]
    ...
    for i in range(0, len(grain_types)):
        ...
        if grain_types[i] == 'graphite':
            ...
            Imperp = np.interp(wavelengths[::-1],data_perp['table'][::-1]['wavelength_um'], data_perp['table']['Im_n'][::-1])
            Impar = np.interp(wavelengths[::-1],data_par['table'][::-1]['wavelength_um'], data_par['table']['Im_n'][::-1])
            Imperp = Imperp[::-1]
            Impar = Impar[::-1]
        elif grain_types[i] == 'silicate':
            data = read_dielectric_file('draine_lee_1984/eps_suvSil')
            Im = np.interp(wavelengths[::-1],data['table'][::-1]['wavelength_um'], data['table']['Im_n'][::-1])
            Im = Im[::-1]
        ...
        if grain_types[i] == 'graphite':
            for j in range(0, nE):
                yields[j] = photoelectric_yield_graphite(graphite_work_function,Z[i],
                                                         a[i]*1e3,electron_escape_length,
                                                         photon_energies[j],wavelengths[j]*1e3,
                                                         Imperp[j],Impar[j])
        elif grain_types[i] == 'silicate':
            for j in range(0, nE):
    ...
    fig.savefig('dust_photoelectric_yields.pdf', format='pdf', dpi=300)


def compare_yield_functions(size_micron=0.0004, Z=0, E_min=4.0, E_max=30.0, nE=300, savefile=None):
    """
    ...
    ----------
    size_micron : float
        Grain radius in microns (default 0.0004 micron = 4e-4 um).
    Z : int
    ...
    """
    from dust_charging import photoelectric_yield_graphite_vec, photoelectric_yield_silicate_vec

    # energy grid
    E = np.linspace(E_min, E_max, nE)  # eV
    # wavelengths in microns and nm used by scalar functions
    wav_micron = 1.2398 / E
    wav_nm = wav_micron * 1e3

    a_nm = float(size_micron) * 1e3  # micron -> nm (scalar functions expect nm)
    ...
    try:
        ...
        print('Warning: could not read graphite dielectric files:', e)
        Im_perp = np.ones_like(E) * 1e-3
        Im_par = np.ones_like(E) * 1e-3

    # scalar yields (graphite)
    Y_scalar_g = np.zeros_like(E)
    for i, Ei in enumerate(E):
        Y_scalar_g[i] = photoelectric_yield_graphite(graphite_work_function, Z, a_nm, electron_escape_length,
                                                   Ei, wav_nm[i], Im_perp[i], Im_par[i])

    # vectorized yields (graphite) - vectorized expects a in nm and wav in nm
    Y_vec_g = photoelectric_yield_graphite_vec(graphite_work_function, np.array([Z]), a_nm, electron_escape_length, E, wav_nm, Im_perp, Im_par)
    ...
    ax.set_title(f'Graphite a={size_micron} µm, Z={Z}')
    ax.set_yscale('log')
    ax.legend()
    ax.grid(True, ls=':')
    ...
    try:
        ...
        print('Warning: could not read silicate dielectric file:', e)
        Im_sil = np.ones_like(E) * 1e-3

    # scalar yields (silicate)
    Y_scalar_s = np.zeros_like(E)
    for i, Ei in enumerate(E):
        Y_scalar_s[i] = photoelectric_yield_silicate(silicate_work_function, Z, a_nm, electron_escape_length,
                                                   Ei, wav_nm[i], Im_sil[i])

    # vectorized yields (silicate)
    Y_vec_s = photoelectric_yield_silicate_vec(silicate_work_function, np.array([Z]), a_nm, electron_escape_length, E, wav_nm, Im_sil)
    Y_vec_s = Y_vec_s[:, 0]
    ...
    ax.set_xlabel('Photon energy (eV)')
    ax.set_ylabel('Yield Y')
    ax.set_title(f'Silicate a={size_micron} µm, Z={Z}')
    ax.set_yscale('log')
    ax.legend()
    ax.grid(True, ls=':')

    fig.tight_layout()
    out = savefile or f'yield_comparison_{size_micron:.6f}um_Z{Z}.pdf'
...
def compute_photoelectric_heating_rate(args):
    ...
    Gamma = np.trapz(dGamma, radiation_field[:,0])
    ...
    if Z < 0:
        ...
        Gamma_det = np.trapz(dGamma_det, radiation_field[:,0])
        Gamma += Gamma_det

    return Gamma 


def compute_photoelectric_heating_rate_single_bin(Z, a_micron, E_eV, I_E_surface,
                                                 grain_type='graphite',
    ...
    a_nm : float
        Grain radius in nanometres (nm). Matches internal unit convention in this file.
    E_eV : float
    ...
    a_micron : float or None
        Grain radius in microns. If provided and C_abs_cm2 is None, the function
    ...
    wav_nm = (1.2398 / E) * 1e3  # convert E_eV -> wavelength nm (hc/e ~ 1239.8 eV*nm)
    ...
    if grain_type.lower().startswith('gra'):
        if Imperp_val is None or Impar_val is None:
            try:
                ...
                raise RuntimeError('Failed to read/interpolate graphite dielectric files: ' + str(_ex))
        # call scalar yield function
        Y = photoelectric_yield_graphite(graphite_work_function, Z, a_nm, electron_escape_length, E, wav_nm, Imperp_val, Impar_val)
    else:
        # silicate
        if Im_val is None:
            try:
                ...
                raise RuntimeError('Failed to read/interpolate silicate dielectric file: ' + str(_ex))
        Y = photoelectric_yield_silicate(silicate_work_function, Z, a_nm, electron_escape_length, E, wav_nm, Im_val)
    ...
    Emin = min_energy_ejection(Z, a_nm)
    if grain_type.lower().startswith('gra'):
    ...
    y2 = escape_fraction_attempting_electrons(E, Emin_ej, Z, a_nm)
    if y2 == 0.0:
        integral_fE = 0.0
    else:
        integral_fE = attempting_electron_energy_integral(E, Emin, Emin_ej, Z, a_nm) / y2
        if integral_fE < 0.0:
            integral_fE = 0.0
    ...
    if Z < 0:
        if grain_type.lower().startswith('gra'):
            E_pdt = photodetachment_energy_graphite(Z, a_nm)
        else:
            E_pdt = photodetachment_energy_silicate(Z, a_nm)
    ...
    if grain_type.lower().startswith('gra'):
        la = photon_attenuation_length_graphite(wav_nm, Imperp_val, Impar_val)
    else:
        la = photon_attenuation_length_silicate(wav_nm, Im_val)
...
def compute_recombination_cooling_rate(args):
    ...
    ltilde = DS87_lambda_function(Z,-1.,a*1e-6,T)
    if grain_type == 'graphite':
        s = electron_sticking_coefficient_graphite(Z,a*1e3)
    elif grain_type == 'silicate':
        s = electron_sticking_coefficient_silicate(Z,a*1e3)
    recomb_rate = np.pi * (a*1e-4)**2. * ne * s * np.sqrt(8. * kb.to('erg/K').d * T / (np.pi * me)) *\
        ltilde * kb.to('erg/K').d * T
    return recomb_rate
...
def compute_autoionisation_cooling_rate(args):
    ...
    from unyt import kb
    if grain_type == 'graphite':
        EA = electron_affinity_graphite(graphite_work_function,Zmin,a*1e3)
    elif grain_type == 'silicate':
        EA = electron_affinity_silicate(silicate_work_function,Zmin,a*1e3)
    Jtilde = DS87_J_function(Zmin,-1.,a*1e-6,T)
    autoion_rate = np.pi * (a*1e-4)**2. * ne * prob_Zmin * np.sqrt(8. * kb.to('erg/K').d * T / (np.pi * me)) *\
        Jtilde * EA * eV2erg
    return autoion_rate

def DS87_lambda_function(Z,q,a,T):
    """
    Numeric implementation of the DS87 "lambda~" function.

    Units contract:
      - a: grain radius in meters (consistent with DS87_J_function_vec)
      - T: temperature in K
      - Z, q: unitless charges

    The function computes nu = Z/q and tau = a(cm) * k_B(erg/K) * T / ( (q*e_statC)^2 )
    and returns the dimensionless ltilde value per Draine & Sutin (1987).
    This mirrors the unit choices used in `DS87_J_function_vec` in `dust_charging.py`.
    """
    ...
    # convert a (meters) to cm for cgs expression
...
def DS87_J_function(Z,q,a,T):
    """
    Scalar-friendly numeric implementation of DS87 J-tilde.
    Matches the vectorized `DS87_J_function_vec` in `dust_charging.py`.
    Inputs:
      - a: grain radius in meters
      - Z, q: unitless
      - T: temperature in K
    Returns J (scalar or array) dimensionless.
    """
    # reuse the lambda implementation's numeric constants and logic
    kb_cgs = 1.380649e-16     # erg / K
    e_statC = 4.8032047e-10   # statcoulomb
...
def plot_DS87_lambda(Z,q,a,Tmin,Tmax,nT=100):
    ...
    """
    import matplotlib.pyplot as plt
    from unyt import m,K,statC,kb

    temperatures = np.logspace(np.log10(Tmin),np.log10(Tmax),nT)
    ltilde_values = [DS87_lambda_function(Z,q,a,T) for T in temperatures]
    tau = a * m * kb * temperatures * K / (q * 4.8032047e-10*statC)**2.

    data = np.loadtxt('lambda_0.csv', delimiter=',')
    Nc_Draine = data[:, 0]
    IPV0_Draine = data[:, 1]
    ...
    plt.legend()
    plt.grid()
    plt.savefig('DS87_lambda_function.pdf', format='pdf', dpi=300)

def autoionisation_potential_graphite(a):
    # Eq 23 in Weingartner & Draine 2001
    return 3.9 + 0.12 * a + 2. / a

def autoionisation_potential_silicate(a):
    # Eq 23 in Weingartner & Draine 2001
    return 2.5 + 0.07 * a + 8. / a

def most_negative_allowed_charge_graphite(a):
    # Eq 24 in Weingartner & Draine 2001
    import math
    U_ait = autoionisation_potential_graphite(a)
    return math.floor(-U_ait / 14.4 * a) + 1.
...
def electron_sticking_coefficient_graphite(Z,a):
    if Z == 0:
        Nc = 468. * a**3.
        s = 0.5 * (1. - np.exp(-a/electron_escape_length)) * 1. / (1. + np.exp(20.-Nc))
    elif Z < 0:
        Zmin = most_negative_allowed_charge_graphite(a*10)
        if Z > Zmin:
            Nc = 468. * a**3.
            s = 0.5 * (1. - np.exp(-a/electron_escape_length)) * 1. / (1. + np.exp(20.-Nc))
        else:
        ...
        s = 0.5 * (1. - np.exp(-a/electron_escape_length))
    return s
...
def electron_sticking_coefficient_silicate(Z,a):
    if Z == 0:
        ...
        Zmin = most_negative_allowed_charge_silicate(a*10)
        if Z > Zmin:
            Nc = 468. * a**3.
            s = 0.5 * (1. - np.exp(-a/electron_escape_length)) * 1. / (1. + np.exp(20.-Nc))
        else:
...
def most_positive_allowed_charge(a,W,Emax):
    # Eq 22 in Weingartner & Draine 2001
    import math
    Zmax = ((Emax - W)/14.4 * a + 0.5 - 0.3/a)/(1. + 0.3/a)
    return math.floor(Zmax)
...
def plot_peh_vs_recombination(grain_types,a,G0,ne,Tmin,Tmax,nT=100,radiation_model='Draine', use_equilibrium=False):
    ...
    from PAH_photoelectric_heating import blackbody_radiation
    from dust_emission import compute_cross_sections
    ...
    G0 = np.trapz(2.*np.pi*f.to('W/m**2/eV').d[(E<=13.6)&(E>=5.17)],E[(E<=13.6)&(E>=5.17)]) / 1.68e-6
    ...
    for i in range(0, len(grain_types)):
        ...
        for j in range(0, nT):
            ...
            args = []
            for k in range(0, len(grain_charges)):
                args.append((grain_charges[k],a[i],ne,temperatures[j],grain_types[i]))
            with concurrent.futures.ProcessPoolExecutor(max_workers=20) as executor:
            ...
            args = (grain_mc,a[i],ne,temperatures[j],grain_types[i])
    ...
    fig.savefig(f'dust_photoelectric_heating_vs_recombination_{radiation_model}.pdf', format='pdf', dpi=300)            
...
def plot_DS87_thetanu():
    ...
    fig.savefig(f'DS87_thetanu.pdf', format='pdf', dpi=300)
...
def compute_peh_model(grain_type, radiation_model, a, ne, T, n_gamma=10, sweep_variable='G0'):
    """
    ...
    from dust_charging import equilibrium_charge_for_grain, compute_G0_from_rad_field
    ...
    for i in range(0, n_gamma):
        ...
        Zs_eq, P_eq, rates, Zmean, Zsigma = equilibrium_charge_for_grain(
            G0_used, ne_used, T_used, grain_type, a,
...
def compute_tables_ISRF(ne, T, n_gamma=10, radiation_model='Mathis', sweep_variable='G0'):
    ...
    grain_types = [('graphite',1e-2,'steelblue','Gra'),('graphite',1e-1,'cornflowerblue','Gra'),
                   ('silicate',5e-3,'saddlebrown','suvSil'),('silicate',1e-1,'sandybrown','suvSil')]
    ...
    for grain_type, a, color, name in grain_types:
        gamma, E_abs_arr, peh_rate, rec_rate = compute_peh_model(grain_type, radiation_model, a, ne, T, n_gamma=n_gamma, sweep_variable=sweep_variable)
        print('gamma:', gamma)
        ...
        ax.plot(gamma, peh_rate, label=fr'{grain_type}, $a={a}$ $\\mu$m', color=color, linestyle='-', linewidth=2)
        ax.plot(gamma, rec_rate, label=fr'{grain_type}, $a={a}$ $\\mu$m', color=color, linestyle='--', linewidth=2)

        # 4. Save the results to a file
        if not os.path.exists('dust_PEH_tables'):
            os.makedirs('dust_PEH_tables')
    ...
    np.savetxt(f'dust_PEH_tables/dust_PEH_{a:.4f}_micron_{name}_sweep-{sweep_variable}.txt', outdata, header=header, fmt='%14.6e', comments='')
    ...
    ls_handles = []
    ls_labels = []
    for grain_type, a, color, dustname in grain_types:
        ls_handles.append(Line2D([0], [0], color=color, linestyle='-', lw=2))
        ls_labels.append(fr'{grain_type}, $a={a}$ $\mu$m')
    legend2 = ax.legend(ls_handles, ls_labels, loc='upper right', fontsize=12, frameon=False, title='Grain type')
    ax.add_artist(legend1)
...
def plot_efficiency(T,ne,radiation_model='Draine',G0factor=1.0,nsizes=50):
    ...
    grain_sizes = np.logspace(np.log10(10e-4),np.log10(10000e-4),nsizes) # in cm

    # Read the two column file 1e4_Draine.csv
    data = np.loadtxt('1e4_Draine.csv', delimiter=',')
    ...
    data = np.loadtxt('1e4_silicate_draine.csv', delimiter=',')
    asize_draine = data[:, 0]  # in Angstroms
    efficiency_draine = data[:, 1]
    ...
    for mat in grain_types:
        if mat == 'graphite':
            data = np.loadtxt('Draine_potential_graphite_WNM.csv', delimiter=',', skiprows=1)
            linestyle=':'
        elif mat == 'silicate':
            data = np.loadtxt('Draine_potential_silicate_WNM.csv', delimiter=',', skiprows=1)
            linestyle='-.'
    ...
    for i in range(0, len(grain_types)):
        ...
        for j in range(0, len(grain_sizes)):
            ...
            potentials[j] = Zmean_eq * e / (4.0 * np.pi * epsilon0_SI * grain_sizes[j]*1e-6)
            ...
            eff[j] = float(eff_val)
            print(f'Grain: {grain_types[i]}, a={grain_sizes[j]*1e4} A, PEH rate: {peh_rate:.3e}, Rec rate: {rec_rate:.3e}, Autoionisation rate: {ai_rate:.3e}, E_abs: {E_abs:.3e}, Efficiency: {eff[j]:.3e}')

        # 5. Plot the results for this grain type
        ax.plot(grain_sizes * 1e4, eff, label=fr'{grain_types[i]}',
                color=color, linestyle='-', linewidth=2)
        
        ax_pot.plot(grain_sizes * 1e4, potentials, label=fr'{grain_types[i]}',
                color=color, linestyle='-', linewidth=2)
    ...
    fig.savefig(f'dust_photoelectric_heating_efficiency_{radiation_model}.pdf', format='pdf', dpi=300)

    fig_pot.subplots_adjust(top=0.97, bottom=0.11, left=0.13, right=0.98, wspace=0, hspace=0)
    fig_pot.savefig(f'dust_photoelectric_heating_potential_{radiation_model}.pdf', format='pdf', dpi=300)
...
def plot_efficiency_all_fields(T, ne, G0factor=1.0, nsizes=50,
    ...
    fig_sed.savefig(f'radiation_fields_comparison_T{int(T)}_ne{ne:.1e}.pdf', format='pdf', dpi=300)

    grain_sizes = np.logspace(np.log10(10e-4), np.log10(10000e-4), nsizes)  # cm
    ...
    fig.savefig(f'dust_photoelectric_heating_efficiency_multiISRF_T{int(T)}_ne{ne:.1e}.pdf',
                format='pdf', dpi=300)
...
def _compute_rates_point(task):
    """Helper for parallel execution: task is a tuple (G0_used, ne_used, T_used, grain_type, a_micron, radiation_model).
    Returns (peh, rec) as floats (or nan on error).
    """
    try:
        G0_used, ne_used, T_used, grain_type, a_micron, radiation_model = task
        from dust_charging import equilibrium_charge_for_grain
        Zs, P, rates, Zmean, Zsigma = equilibrium_charge_for_grain(
            float(G0_used), float(ne_used), float(T_used), grain_type, a_micron,
            radiation_model=radiation_model, rad_field=None, yield_params=None, Z_start=0, debug=False)
        peh = float(rates.get('Gamma_total', np.nan))
        rec = float(rates.get('Recomb_total', 0.0)) + float(rates.get('Autoionisation_cooling', 0.0))
        return peh, rec, Zmean, Zsigma
    except Exception:
        return np.nan, np.nan


def make_rate_gamma_T_tables(grain_type, a_micron, radiation_model='Mathis',
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=num_workers) as exe:
        ...
        for pos, t in enumerate(tasks):
            fut = exe.submit(_compute_rates_point, (t[2], t[3], t[4], grain_type, a_micron, radiation_model))
    ...
    fill_bad = -1e30
    with np.errstate(divide='ignore', invalid='ignore'):
        log_peh = np.log10(np.abs(peh_mat))
        log_rec = np.log10(np.abs(rec_mat))
    ...
    size_tag = f"{a_micron:.4f}_micron"
...
def plot_heating_cooling_surfaces(grain_type, a_micron, radiation_model='Mathis', combination='G0_vs_sqrtT_over_ne',
    ...
    for j, y in enumerate(y_vals):
        for i, x in enumerate(x_vals):
            ...
            tasks.append((G0_used, ne_used, T_used, grain_type, a_micron, radiation_model))
    ...
    prefix = save_prefix or f'heating_cooling_{grain_type}_{a_micron:.4f}_{combination}'
    outname = prefix + '.pdf'
...
def plot_average_potential(radiation_model='Mathis', G0factor=1.0, ne=1.0, T=100.0,
    ...
    try:
        from dust_charging import compute_G0_from_rad_field
    ...
    for mat_idx, mat in enumerate(materials):
        ...
        for idx, aA in enumerate(fallback_sizes):
            a_micron = aA * 1e-4  # Angstrom -> microns
            a_m = a_micron * 1e-6     
    ...
    out = savefile or f'avg_potential_{radiation_model}.pdf'
    fig.savefig(out, dpi=200)
    plt.close(fig)
    ...
    for mat in materials:
        ...
        for aA in sizes_A:
            ...
            num = np.trapz(radI_sub[::-1] * C_interp, radE_sub[::-1])
            den = np.trapz(radI_sub[::-1], radE_sub[::-1])
            if den <= 0:
                avgC.append(0.0)
            else:
                avgC.append(num / den)
    ...
    ax2.legend()
    fig2.tight_layout()
    out2 = savefile.replace('.pdf', f'_fluxnorm_{radiation_model}.pdf') if savefile else f'avg_potential_{radiation_model}_fluxnorm.pdf'
...
def plot_csa_IM19():
    ...
    ax.legend(loc='upper right', fontsize=12, frameon=False)
    fig.savefig(f'dust_absorption_cs_IM19.pdf', format='pdf', dpi=300)
...
def compare_Mathis_WD01(savefile=None, E_min=0.1, E_max=13.6, nE=1000, norm_band=(5.17,13.6)):
    ...
    try:
        ...
        raise RuntimeError(f'Could not obtain Mathis functional field: {e}')
    ...
    total_u_file = np.trapz(u_E_file, E)
    total_u_WD = np.trapz(u_E_WD, E)
    ...
    if np.any(band_mask):
        total_u_file_band = np.trapz(u_E_file[band_mask], E[band_mask])
        total_u_WD_band = np.trapz(u_E_WD[band_mask], E[band_mask])
    else:
    ...
    out = savefile or 'compare_Mathis_WD01_absolute.pdf'
    fig.tight_layout()
    fig.savefig(out, dpi=200)
    plt.close(fig)
    return out
...
def diagnose_Mathis_blackbody_models(E_min=0.01, E_max=13.6, nE=2000):
    ...
    band_mask = (E >= 6.0) & (E <= 13.6)
    total_u_A_band = np.trapz(results['u_E_A'][band_mask], E[band_mask])
    total_u_B_band = np.trapz(results['u_E_B'][band_mask], E[band_mask])
    total_u_file_band = np.trapz(results['u_E_file'][band_mask], E[band_mask])
...
def plot_heating_cooling_3d(grain_type, a_micron, radiation_model='Mathis', pair='G0_ne',
                           n_vals=32, rate='heating', savefile=None, show=True,
                           num_workers=None, T_groups=None):
    ...
    if savefile:
        fig.savefig(savefile or f'heating_cooling_3d_Tgroups_{grain_type}_{a_micron:.4f}.pdf', dpi=200)
    if show:
        _plt.show()
    _plt.close(fig)
...
def plot_heating_cooling_cartesian(grain_type, a_micron, radiation_model='Mathis',
                                  G0_vals=None, ne_vals=None, sqrtT_vals=None,
                                  rate='heating', save_prefix=None, show=True,
                                  num_workers=None, n_vals=10):
    ...
    for g0 in G0_vals:
        for ne in ne_vals:
            for sqrtT in sqrtT_vals:
                ...
                tasks.append((float(g0), float(ne), T, grain_type, a_micron, radiation_model))
    ...
    if save_prefix:
        outname = f'{save_prefix}_cartesian_{grain_type}_{a_micron:.4f}.pdf'
        fig.savefig(outname, dpi=200)
...
def plot_heating_cooling_cartesian_3d(grain_type, a_micron, radiation_model='Mathis',
    ...
    if save_prefix:
        outname = f'{save_prefix}_cartesian_3d_{grain_type}_{a_micron:.4f}.pdf'
        fig.savefig(outname, dpi=200)
...
def plot_gamma_combo_projections(grain_type, a_micron, radiation_model='Mathis',
                                 G0_vals=None, ne_vals=None, T_vals=None,
                                 rate='heating', save_prefix=None, show=True,
                                 num_workers=None, n_vals=12, nbins=50):
    ...
    for g0 in G0_vals:
        for ne in ne_vals:
            for T in T_vals:
                tasks.append((float(g0), float(ne), float(T), grain_type, a_micron, radiation_model))
                metas.append((g0, ne, T))
    ...
    if save_prefix:
        outname = f'{save_prefix}_gamma_combo_{grain_type}_{a_micron:.4f}.pdf'
        fig.savefig(outname, dpi=200)
...
def plot_gamma_surfaces(grain_type, a_micron, radiation_model='Mathis',
                        G0_vals=None, ne_vals=None, T_vals=None,
                        rate='heating', save_prefix=None, show=True,
                        num_workers=None, n_vals=12, cmap='viridis'):
    ...
    if save_prefix:
        outname = f'{save_prefix}_gamma_surfaces_{grain_type}_{a_micron:.4f}.pdf'
        fig.savefig(outname, dpi=200)
...
def plot_rate_vs_gamma_for_T(grain_type, a_micron, radiation_model='Mathis',
                             mode='fix_G0', fixed_value=1.0,
                             gamma_vals=None, T_groups=None,
                             rate='heating', save_prefix=None, show=True,
                             num_workers=None, n_gamma=50):
    ...
    for T in T_groups:
        ...
        for gamma in gamma_vals:
            ...
            tasks.append((G0, ne, float(T), grain_type, a_micron, radiation_model))
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=num_workers) as exe:
        ...
        for idx, fut in enumerate(iterator):
            try:
                ...
                peh, rec, Zmean, Zsigma = np.nan, np.nan, np.nan, np.nan
            peh_vals[idx] = peh
            rec_vals[idx] = rec
    ...
    if save_prefix:
        outname = f'{save_prefix}_rate_vs_gamma_{mode}_{grain_type}_{a_micron:.4f}.pdf'
        fig.savefig(outname, dpi=200)
