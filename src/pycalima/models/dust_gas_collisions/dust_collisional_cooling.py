...
def compute_efficiency(args):
    ...
    # Unpack args, with optional electrostatic parameters
    if len(args) == 10:
        ...
        T,a_dust,s_dust,ne_val,Zi,Zd,Md,Mi,nv,delta_max,phi_eV = args
        Zi_charge = 0
        a_dust_radius = None
    else:
        T,a_dust,s_dust,ne_val,Zi,Zd,Md,Mi,nv,delta_max,Zi_charge,a_dust_radius = args
        phi_eV = None
    ...
    for i in range(0, nv):
        ...
        # Preferred mode: explicit phi shift in eV (same convention as sputtering tables).
        if phi_eV is not None:
            Ei_eff = Ei + phi_eV * eV2erg
        else:
            # Backward-compatible mode for older callers.
            if Zi_charge != 0 and a_dust_radius is not None:
                E_charge = (Zi_charge * Zd * elem_charge**2. / a_dust_radius) / eV2erg
                Ei_eff = Ei - E_charge
            else:
                Ei_eff = Ei
        ...
        agg = n_dust,ne_val,a_dust,Zi,Zd,Md,Mi,Ei_eff,delta_max
        E_imp,_,_,_ = compute_energy_deposited(agg)
        h[i] = xi**2. * (E_imp/Ei_eff) * np.exp(-xi)
...
def _build_deposited_energy_lookup_ion(n_dust, ne_val, a_dust, Zi, Zd, Md, Mi, 
                                        delta_max, E_min_eV, E_max_eV, nE_lookup=512):
    """Precompute deposited energy as function of initial kinetic energy.
    
    Returns E_lookup (energy grid in eV) and E_imp_lookup (deposited energy array).
    """
    ...
    E_imp_lookup = np.zeros(nE, dtype=float)
    
    for iE, Ei_eV in enumerate(E_lookup):
        Ei_erg = Ei_eV * eV2erg
        agg = n_dust, ne_val, a_dust, Zi, Zd, Md, Mi, Ei_erg, delta_max
        E_imp, _, _, _ = compute_energy_deposited(agg)
        E_imp_lookup[iE] = E_imp / eV2erg
    
    return E_lookup, E_imp_lookup


def _build_deposited_energy_lookup_electron(fit_params, a_dust, delta_max, 
                                             E_min_eV, E_max_eV, nE_lookup=512):
    """Precompute deposited energy as function of initial kinetic energy (electrons).
    ...
    for iE, Ei_eV in enumerate(E_lookup):
        agg = fit_params, a_dust, Ei_eV, delta_max
        E_imp, _, _, _ = compute_energy_deposited_electron(agg)
        # compute_energy_deposited_electron returns energies in eV when input is in eV
        E_imp_lookup[iE] = E_imp
    
    return E_lookup, E_imp_lookup


def compute_efficiency_batch_phi(args):
    """Compute ion collisional efficiency for all phi values at fixed T.

    This reuses the same velocity/x grid for every phi entry and returns
    a full 1D array h(phi), reducing repeated setup overhead.
    Uses precomputed deposited-energy lookup for fast interpolation.
    """

    T,a_dust,s_dust,ne_val,Zi,Zd,Md,Mi,nv,delta_max,phi_grid,E_lookup,E_imp_lookup = args
    phi_grid = np.asarray(phi_grid, dtype=float)
    use_lookup = E_lookup is not None and E_imp_lookup is not None

    # 1. Determine the velocity/x integration range once for this T.
    v_0 = np.sqrt(2.*kb*T/Mi) * (1. - (np.sqrt(3./2.) - 1.))
    while Maxwell_Boltzmann_function(v_0,Mi,T) > 1e-30:
        v_0 = v_0 / 2.
    x_0 = 0.5 * Mi * v_0**2. / (kb*T)

    v_max = np.sqrt(2. * kb * 1e10 / Mi)
    while Maxwell_Boltzmann_function(v_max,Mi,T) < 1e-30:
        v_max = v_max / 2.0
    x_max = 0.5 * Mi * v_max**2. / (kb*T)

    x = np.logspace(np.log10(x_0), np.log10(x_max), nv)
    x_weight = x**2.0 * np.exp(-x)
    e_kin = x * kb * T

    n_dust = s_dust / Md * Zd
    h_phi = np.zeros(len(phi_grid), dtype=float)

    # 2. Loop over phi using the same kinetic-energy grid.
    for jphi, phi_eV in enumerate(phi_grid):
        e_eff = e_kin + phi_eV * eV2erg
        h_integrand = np.zeros(nv, dtype=float)
        valid_idx = np.where(e_eff > 0.0)[0]

        if use_lookup:
            # Use precomputed lookup table with interpolation (fast).
            E_eff_eV = e_eff[valid_idx] / eV2erg
            E_imp_interp = np.interp(E_eff_eV, E_lookup, E_imp_lookup, left=0.0, right=E_imp_lookup[-1])
            h_integrand[valid_idx] = x_weight[valid_idx] * (E_imp_interp / E_eff_eV)
        else:
            # Fallback: compute deposited energy for each point (slow).
            for idx in valid_idx:
                agg = n_dust,ne_val,a_dust,Zi,Zd,Md,Mi,e_eff[idx],delta_max
                E_imp,_,_,_ = compute_energy_deposited(agg)
                h_integrand[idx] = x_weight[idx] * (E_imp / e_eff[idx])

        h_phi[jphi] = 0.5 * trapezoid(h_integrand, x)
...
def compute_efficiency_electron(args):
    ...
    if len(args) == 5:
        ...
        T,a_dust,fit_params,nv,delta_max,phi_eV = args
        Z_dust = None
        a_dust_radius = None
    else:
        T,a_dust,fit_params,nv,delta_max,Z_dust,a_dust_radius = args
        phi_eV = None
    ...
    for i in range(0, nv):
        ...
        if phi_eV is not None:
            ...
            # Backward-compatible mode for older callers.
            if Z_dust is not None and a_dust_radius is not None and a_dust_radius > 0:
                E_charge = ((-1) * Z_dust * elem_charge**2. / a_dust_radius) / eV2erg
                Ei_eff = Ei - E_charge
            else:
                Ei_eff = Ei
        ...
        agg = fit_params,a_dust,Ei_eff/eV2erg,delta_max
        E_imp,_,_,_ = compute_energy_deposited_electron(agg)
        h[i] = xi**2. * (E_imp*eV2erg/Ei_eff) * np.exp(-xi)
...
def compute_efficiency_electron_batch_phi(args):
    """Compute electron collisional efficiency for all phi values at fixed T.
    ...
    T,a_dust,fit_params,nv,delta_max,phi_grid,E_lookup,E_imp_lookup = args
    phi_grid = np.asarray(phi_grid, dtype=float)
    me = 9.10938e-28
    use_lookup = E_lookup is not None and E_imp_lookup is not None
    ...
    for jphi, phi_eV in enumerate(phi_grid):
        ...
        if use_lookup:
            ...
            # Fallback: compute deposited energy for each point (slow).
            for idx in valid_idx:
                agg = fit_params,a_dust,e_eff[idx]/eV2erg,delta_max
                E_imp,_,_,_ = compute_energy_deposited_electron(agg)
                h_integrand[idx] = x_weight[idx] * (E_imp * eV2erg / e_eff[idx])
...
def Dwek1981_electron_efficiency(a,T):
    x = 2.71e8 * a **(2./3.)/T
    h = np.zeros_like(x)
...
def test_efficiency_electron(Tmin,Tmax,nT=100,nv=300,delta_max=0.1):
    ...
    fig, ax = plt.subplots(1,1, figsize=(5,4),dpi=300,facecolor='w',edgecolor='k',sharey=True)
    ax.set_xlabel(r'$T$ [K]', fontsize=16)
    ax.set_ylabel(r'$h_e(a,T)$',fontsize=16)
    ax.tick_params(labelsize=14)
    ax.set_yscale('log')
    ax.set_xscale('log')
    ax.xaxis.set_ticks_position('both')
    ...
    # 2. Set the temperature and heating rate arrays
    Tgas = np.logspace(np.log10(Tmin),np.log10(Tmax),nT)
    a_dust_cm = dust_model.basic_a0[5]*1e-4
    a_dust_micron = dust_model.basic_a0[5]
    fit_params = [-0.000423375,-3.57429e-11,-3.37861e-7,-3.18688,-0.587928,-0.000232675,1.53851,1.41476]
    args_list = [(Ti,a_dust_cm,fit_params,nv,delta_max) for Ti in Tgas]
    ...
    # 4. Plot the efficiency
    ax.plot(Tgas,h_eff,color='saddlebrown',linewidth=2,label='smallSil')

    # 5. Plot the efficiency given by Dwek & Werner (1981) - equation A12 in their appendix
    ax.plot(Tgas,Dwek1981_electron_efficiency(a_dust_micron,Tgas),color='saddlebrown',linewidth=2,linestyle=':')

    ax.legend(loc='best',frameon=False,fontsize=12)
    fig.subplots_adjust(top=0.99,bottom=0.13,left=0.16,right=0.97,hspace=0,wspace=0)
    fig.savefig('testing_efficiency_electron.png',format='png')
...
def plot_collisional_cooling_simple(Tmin,Tmax,Td,nT=100,nv=300,delta_max=0.1):
    ...
    ax.set_xlabel(r'$T$ [K]', fontsize=16)
    ax.set_ylabel(r'$h(\vec{\mathcal{P}}_j,T)$ [erg/s]',fontsize=16)
    ax.tick_params(labelsize=14)
    ax.set_yscale('log')
    ax.set_xscale('log')
    ...
    # 2. Plot the fitting function by Dwek and Werner (1981)
    ax.plot(Tgas,DwekWerner81_cooling(a_dust_micron,Tgas),linestyle='--',color='k',label=r'DW81',lw=2)
    ...
    args_list = [(Ti,a_dust_cm,popt_sil,nv,delta_max) for Ti in Tgas]
    ...
    h_electron = np.array(results)
    H_electron = np.sqrt(32./(np.pi*me)) * np.pi * a_dust_cm**2. * np.sqrt(kb*Tgas)*kb*(Tgas-Td) * h_electron
...
def export_collisional_cooling(Tmin,Tmax,
                                grain_size_micron,
                                grain_density,
                                grain_atomic_mass,
                                grain_atomic_number,
    ...
    Args:
        Tmin (float): minimum gas temperature [K]
        Tmax (float): maximum gas temperature [K]
        grain_size_micron (float): grain size [micron]
        grain_density (float): grain density [g/cm^3]
        grain_atomic_mass (float): grain average atomic mass [a.u.]
        grain_atomic_number (float): grain average atomic number
        dust_label (str): label for the dust grain type (e.g., 'Gra', 'Sil')
        ion_atomic_masses (np.ndarray): ion atomic masses [a.u.]
        ion_atomic_numbers (np.ndarray): ion atomic numbers
        nZ_ion (np.ndarray): number of charge states for each ion (determines charge range)
        grain_ne_val (float): effective number of electrons per dust atom for electronic stopping
        electron_stopping_fit_params (tuple): fitting parameters for electron stopping power
        nT (int): number of temperature bins
        nv (int): number of velocity bins
        delta_max (float): maximum energy step fraction
        nphi (int): number of phi (grain charge) bins
        table_dir (str): directory for output tables

    Returns:
        None (exports tables to files)
    """
    from unyt import g,cm,eV
    from scipy.optimize import curve_fit
    
    # 1. Setup default ion species if not provided (H, He, C)
    if ion_atomic_masses is None:
        ion_atomic_masses = np.array([1.00784, 4.002602, 12.011])  # H, He, C
        ion_atomic_numbers = np.array([1, 2, 6])
        nZ_ion = np.array([1, 2, 6])  # max charge for each ion
    else:
        ion_atomic_masses = np.atleast_1d(ion_atomic_masses)
        ion_atomic_numbers = np.atleast_1d(ion_atomic_numbers)
        nZ_ion = np.atleast_1d(nZ_ion)
    
    # 2. Setup the variables and arrays
    me = 9.10938e-28 # [g]
    Tgas = np.logspace(np.log10(Tmin),np.log10(Tmax),nT)
    a_dust = grain_size_micron * 1e-4  # [cm]
    s_dust = grain_density  # [g/cm^3]
    an_dust = grain_atomic_number
    am_dust = grain_atomic_mass * au2cgs_m  # [g]
    n_dust = s_dust / (am_dust/au2cgs_m) * an_dust

    # Keep consistency with the legacy plotting functions.
    # If not provided, use composition-based defaults used elsewhere in this file.
    if grain_ne_val is None:
        if an_dust <= 6.5:
            grain_ne_val = 4.0
        else:
            grain_ne_val = (2 + 8 + 4 + 4*6) / 7.0
    ...
    if an_dust <= 6.5:
        Zg_min = int(np.floor(most_negative_allowed_charge_graphite(a_dust)))
        Zg_max = int(np.floor(most_positive_allowed_charge(a_dust, graphite_work_function, 13.6)))
    else:
        Zg_min = int(np.floor(most_negative_allowed_charge_silicate(a_dust)))
        Zg_max = int(np.floor(most_positive_allowed_charge(a_dust, silicate_work_function, 13.6)))

    Zk_min = 0
    Zk_max = int(np.max(nZ_ion))
    ...
    if electron_stopping_fit_params is None:
        print("    Computing electron stopping power fitting...")
        E = np.array([15, 20, 30, 40, 60, 80, 100, 150, 200, 300, 400, 600, 800, 1000, 2000, 4000, 6000, 8000, 10000])
        S_prime = np.array([2.40, 6.64, 26.2, 55.1, 105, 128, 137, 141, 137, 127, 117, 99.6, 87.5, 78.1, 52.8, 33.5, 25.1, 20.4, 17.4])
        ...
        popt_sil,_ = curve_fit(stopping_fit,E,S.to('eV/Angstrom').d)

        if data_carbon is not None:
            popt_car,_ = curve_fit(stopping_fit,data_carbon['E']*1e3,data_carbon['S'],
                                   p0=[-0.000423375,-3.57429e-11,-3.37861e-7,-3.18688,
                                       -0.587928,-0.000232675,1.53851,1.41476])
        else:
            popt_car = popt_sil.copy()

        # Composition-consistent choice (same spirit as legacy plot routines).
        if an_dust <= 6.5:
            popt_electron = popt_car
        else:
            popt_electron = popt_sil
        
        print(f"    Electron stopping power fit params: {popt_electron}")
        electron_stopping_fit_params = popt_electron
    
    # 4. Create the directory for the table data
    ...
    # 5. ELECTRON COOLING - compute 2D table (T, phi)
    print(f"    Calculating electron cooling rate for {dust_label}...")
    E_lookup_el, E_imp_lookup_el = _build_deposited_energy_lookup_electron(
        electron_stopping_fit_params, a_dust, delta_max, 1e-8, 1e7, nE_lookup=512
    )
    print(f"        Electron lookup: {len(E_lookup_el)} points from {E_lookup_el[0]:.3e} to {E_lookup_el[-1]:.3e} eV")
    args_list = [(Ti, a_dust, electron_stopping_fit_params, nv, delta_max, phi_grid,
    ...
    h_electron_2d = np.asarray(results, dtype=float)
    if h_electron_2d.shape != (nT, nphi):
        h_electron_2d = h_electron_2d.reshape(nT, nphi)

    # Convert to cooling rate and save 2D table
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        for iT, Ti in enumerate(Tgas):
            row = [f'{np.log10(Ti):.8e}'] + [f'{np.log10(H_electron_2d[iT, j]):.8e}' for j in range(nphi)]
            f.write(' '.join(row) + '\n')
    print(f"        Saved: {output_file}")

    # Store collisional cooling rates for plotting (same quantity as legacy plotting functions).
    rate_data_2d = {'electrons': H_electron_2d}

    # 6. ION COOLING (exactly one file per atomic number)
    for ion_idx, (Mi, Zi) in enumerate(zip(ion_atomic_masses * au2cgs_m, ion_atomic_numbers)):
        Zi = int(Zi)
        ion_name = {1: 'H', 2: 'He', 6: 'C'}.get(int(Zi), f'Z{int(Zi)}')
        print(f"    Calculating cooling rate for {ion_name} (atomic Z={Zi})...")
        # Number density of target atoms [cm^-3], consistent with compute_efficiency.
        n_dust_tmp = s_dust / am_dust * an_dust
        E_lookup_ion, E_imp_lookup_ion = _build_deposited_energy_lookup_ion(
            n_dust_tmp, grain_ne_val, a_dust, Zi, int(an_dust), am_dust, Mi, 
            delta_max, 1e-8, 1e7, nE_lookup=512
        )
        print(f"        Ion lookup (Z={Zi}): {len(E_lookup_ion)} points from {E_lookup_ion[0]:.3e} to {E_lookup_ion[-1]:.3e} eV")
        args_list = [(Ti, a_dust, s_dust, grain_ne_val, Zi, int(an_dust),
                     am_dust, Mi, nv, delta_max, phi_grid,
        ...
        h_ion_2d = np.asarray(results, dtype=float)
        if h_ion_2d.shape != (nT, nphi):
            h_ion_2d = h_ion_2d.reshape(nT, nphi)

        H_ion_2d = np.sqrt(32./(np.pi*Mi)) * np.pi * a_dust**2. * np.sqrt(kb*Tgas[:, np.newaxis]) * kb * h_ion_2d
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            ...
            f.write(' '.join([f'{phi:.8e}' for phi in phi_grid]) + '\n')
            for iT, Ti in enumerate(Tgas):
                row = [f'{np.log10(Ti):.8e}'] + [f'{np.log10(H_ion_2d[iT, j]):.8e}' for j in range(nphi)]
                f.write(' '.join(row) + '\n')
        print(f"        Saved: {output_file}")

        rate_data_2d[ion_name] = H_ion_2d
    
    # 7. CREATE SINGLE-PANEL PLOT with phi families per species
    print("    Creating single-panel efficiency plot with phi families...")
    try:
        import matplotlib.pyplot as plt
        import seaborn as sns

        # Match the visual formatting used in plot_collisional_cooling_simple.
        sns.set_theme(style="white")
        sns.color_palette("Paired")
        sns.set_theme(style="white")
        plt.rcParams.update({
        ...
        species_names = ['electrons'] + [k for k in rate_data_2d.keys() if k != 'electrons']
        fig, ax = plt.subplots(1,1, figsize=(7,4),dpi=300,facecolor='w',edgecolor='k',sharey=True)
        ...
        cmap = plt.cm.tab20
        colors = [cmap(i / max(1, len(species_names)-1)) for i in range(len(species_names))]
        phi_idx_zero = int(np.argmin(np.abs(phi_grid)))

        rng = np.random.default_rng()
        max_phi_curves = 10

        for ispec, name in enumerate(species_names):
            color = colors[ispec]
            H_local = rate_data_2d[name]

            # low-opacity curves for a random subset of phi != 0.
            phi_candidates = np.array([j for j in range(nphi) if j != phi_idx_zero], dtype=int)
            n_draw = min(max_phi_curves, len(phi_candidates))
            if n_draw > 0:
                phi_draw = np.sort(rng.choice(phi_candidates, size=n_draw, replace=False))
            else:
                phi_draw = np.array([], dtype=int)

            for jphi in phi_draw:
                ax.semilogy(Tgas, H_local[:, jphi] * (Tgas-Td), '-', linewidth=0.8, alpha=0.15, color=color)

            # solid line for phi = 0
            if name == 'electrons':
                label = 'e- (phi=0)'
            else:
                label = f'{name} (phi=0)'
            ax.semilogy(Tgas, H_local[:, phi_idx_zero] * (Tgas-Td), '-', linewidth=2.2, color=color, label=label)

        ax.legend(loc='upper left',frameon=False,fontsize=12,ncol=2)
        fig.subplots_adjust(top=0.99,bottom=0.125,left=0.12,right=0.99,hspace=0,wspace=0)
        
        # Save plot
        plot_file = f'{table_dir}/efficiency_curves_{dust_label}.png'
